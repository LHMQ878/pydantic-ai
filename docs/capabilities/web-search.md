...
`allowed_domains`, `max_uses`, Anthropic's `response_inclusion`, and OpenAI Responses'
`external_web_access`. The domain and `max_uses` constraints and `response_inclusion` require native
support (the shipped DuckDuckGo fallback doesn't enforce them). Setting `external_web_access=False`
also requires native support because a local fallback cannot guarantee cached or indexed-only search.
